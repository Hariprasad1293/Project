...
class PatientsDummy(db.Model):
    __tablename__ = 'patientsdummy'
...
def register():
    ...
    new_patient = PatientsDummy(first_name=data['first_name'], last_name=data['last_name'], email=data['email'],
    password=hashed_password, gender=data['gender'], mobile=data['mobile'], address=data['address'])

    db.session.add(new_patient)
    db.session.commit()

    return jsonify({'message': 'New patient created', 'data': data})
